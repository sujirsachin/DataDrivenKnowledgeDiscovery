...
#The predictor US is an qualitative variable with 2 levels "Yes" and "No".Here 'No' class takes the baseline and the UrbanYes is the dummy variable for 'Yes' class. UrbanYes has a low p-value and is statistically significant and the relationship is satisfied. If the store is in US setting the sales are 1200.573 more as compared to compared to sales in stores not in US setting.
...
#In order to reject null hypothesis a predictor must have a low p-value, lower than the t-statistic. So with this assumption, we can reject the null hypothesis for Price and US and we cannot reject it for UrbanYes because the p-value is 0.936 which is high.
...
#Sales=13.03079-0.05448 * Price +1.19964 * USYes+ ??
